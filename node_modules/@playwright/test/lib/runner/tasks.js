...
function createReportBeginTask() {
  ...
  return {
    setup: async ({
      ...
      montonicStartTime = (0, _utils.monotonicTime)();
      reporter.onBegin(rootSuite);
    },
    teardown: async ({
      config
    }) => {
      config.config.metadata.totalTime = (0, _utils.monotonicTime)() - montonicStartTime;
    }
  };
}
...
function createRemoveOutputDirsTask() {
  return {
    setup: async ({
      ...
      await Promise.all(Array.from(outputDirs).map(outputDir => (0, _utilsBundle.rimraf)(outputDir).catch(async error => {
        if (error.code === 'EBUSY') {
          // We failed to remove folder, might be due to the whole folder being mounted inside a container:
          //   https://github.com/microsoft/playwright/issues/12106
          // Do a best-effort to remove all files inside of it instead.
          const entries = await readDirAsync(outputDir).catch(e => []);
          await Promise.all(entries.map(entry => (0, _utilsBundle.rimraf)(_path.default.join(outputDir, entry))));
        } else {
          throw error;
        }
      })));
