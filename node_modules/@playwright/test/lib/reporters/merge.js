...
async function createMergedReport(config, dir, reporterDescriptions) {
...
function parseTestEvents(reportJsonl) {
  return reportJsonl.toString().split('\n').filter(line => line.length).map(line => JSON.parse(line)).filter(event => !commonEvents.has(event.method));
}
...
async function mergeEvents(dir, shardReportFiles, stringPool, printStatus) {
  ...
  return {
    prologue: [mergeConfigureEvents(configureEvents), ...projectEvents, {
      method: 'onBegin',
      params: undefined
    }],
    reports,
...
function mergeConfigureEvents(configureEvents) {
